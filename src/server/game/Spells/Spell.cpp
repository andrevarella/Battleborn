...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    // Instant casts com Arena Preparation (+exceções p profissoes)
    ...
    // don't allow channeled spells / spells with cast time to be casted while moving - (Se tiver andando, nao deixa começar o cast de spells. É necessario dar patch no client para castar andando em spells antigas [remover interrupt_flag]) 83292 = novo scorch
...
void Spell::update(uint32 difftime)
{
    ...
    {
        // don't cancel for melee, autorepeat, triggered and instant spells. - (83292 = novo Scorch) - é necessario dar patch no client para castar andando em spells antigas (remover interrupt_flag)
        if (!IsNextMeleeSwingSpell() && m_spellInfo->Id != 83292 && !IsAutoRepeat() && !IsTriggered())
            cancel(true);
    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    {
        if (m_caster->IsPlayer() && m_caster->ToPlayer()->InBattleground())
        {
            return SPELL_FAILED_NOT_HERE;
